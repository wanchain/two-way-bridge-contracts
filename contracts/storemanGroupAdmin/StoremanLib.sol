...
library StoremanLib {
    ...
    function stakeIn(StoremanType.StoremanData storage data, bytes32 groupId, bytes calldata PK, bytes calldata enodeID) external
    {
        ...
        sk.wkAddr = pkAddr;
        sk.groupId = groupId;
        sk.deposit.total = 0; // = Deposit.Records(0);
        sk.deposit.addRecord(Deposit.Record(StoremanUtil.getDaybyTime(block.timestamp), msg.value));
    ...
    function stakeAppend(StoremanType.StoremanData storage data,  address wkAddr) external  {
        ...
        uint day = StoremanUtil.getDaybyTime(block.timestamp);
    ...
    function checkCanStakeClaimFromGroup(StoremanType.Candidate storage sk, StoremanType.StoremanGroup storage group) private view returns (bool) {
        // 如果group还没选择, 不许提取.
        // group组建失败, 可以提取.
        // 如果已经选择过了, 没选中, 可以提取.
