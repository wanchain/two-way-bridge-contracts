...
interface ICurve {
    ...
    function calPolyCommit(bytes calldata polyCommit, bytes calldata pk) external view returns(uint256 sx, uint256 sy, bool success);
    ...
    function checkSig (bytes32 hash, bytes32 r, bytes32 s, bytes calldata pk) external view returns(bool);
}