...
library RapidityLibV4 {
    ...
    */
    ...
    /// @param value                    Rapidity value
    /// @param userAccount              account of shadow chain, used to receive token
    event UserLockLoggerErc1155(bytes32 indexed smgID, uint indexed tokenPairID, address indexed tokenAccount, uint tokenID, uint value, uint contractFee, bytes userAccount);
    ...
    /// @param value                    Rapidity value
    /// @param userAccount              account of shadow chain, used to receive token
    event UserBurnLoggerErc1155(bytes32 indexed smgID, uint indexed tokenPairID, address indexed tokenAccount, uint tokenID, uint value, uint contractFee, uint fee, bytes userAccount);
    ...
    /// @param tokenAccount             Rapidity shadow token account
    /// @param userAccount              account of original chain, used to receive token
    event SmgMintLoggerErc1155(bytes32 indexed uniqueID, bytes32 indexed smgID, uint indexed tokenPairID, uint tokenID, uint value, uint fee, address tokenAccount, address userAccount);
    ...
    /// @param tokenAccount             Rapidity original token account
    /// @param userAccount              account of original chain, used to receive token
    event SmgReleaseLoggerErc1155(bytes32 indexed uniqueID, bytes32 indexed smgID, uint indexed tokenPairID, uint tokenID, uint value, uint fee, address tokenAccount, address userAccount);

    /// @notice                         mintBridge, user lock token on token original chain
    /// @notice                         event invoked by user mint lock
    /// @param storageData              Cross storage data
    /// @param params                   parameters for user mint lock token on token original chain
    function userLockErc1155(CrossTypesV1.Data storage storageData, RapidityUserLockParams memory params)
    public
    {
        uint fromChainID;
        uint toChainID;
        ...
        if (tokenScAddr == address(0)) {
            ...
            IERC1155(tokenScAddr).safeTransferFrom(msg.sender, address(this), params.tokenID, params.value, '0x0');
        }
        if (left != 0) {
            (msg.sender).transfer(left);
        }
        emit UserLockLoggerErc1155(params.smgID, params.tokenPairID, tokenScAddr, params.tokenID, params.value, contractFee, params.destUserAccount);
    }
    ...
    /// @param storageData              Cross storage data
    /// @param params                   parameters for user burn lock token on token original chain
    function userBurnErc1155(CrossTypesV1.Data storage storageData, RapidityUserBurnParams memory params)
    public
    {
        ITokenManager tokenManager = storageData.tokenManager;
        uint256 contractFee = params.tokenPairContractFee;
        ...
        require(tokenCrossType == uint8(TokenCrossType.ERC1155), "Not ERC1155");
        require(burnShadowTokenErc1155(tokenManager, tokenScAddr, params.tokenID, msg.sender, params.value), "Burn ERC1155 failed");
        ...
        emit UserBurnLoggerErc1155(params.smgID, params.tokenPairID, tokenScAddr, params.tokenID, params.value, contractFee, params.fee, params.destUserAccount);
    }
    ...
    returns (bool) {
        ...
        ITokenManager(tokenManager).burnToken(tokenAddress, userAccount, tokenID, value);
    ...
    /// @param storageData              Cross storage data
    /// @param params                   parameters for storeman mint lock token on token shadow chain
    function smgMintErc1155(CrossTypesV1.Data storage storageData, RapiditySmgMintParams memory params)
    public
    {
        storageData.rapidityTxData.addRapidityTx(params.uniqueID);
        ITokenManager tokenManager = storageData.tokenManager;
        ...
        uint8 tokenCrossType = tokenManager.mapTokenPairType(params.tokenPairID);
        require(tokenCrossType == uint8(TokenCrossType.ERC1155), "Not ERC1155");
        require(mintShadowTokenErc1155(tokenManager, params.destTokenAccount, params.destUserAccount, params.tokenID, params.value), "Mint Erc1155 failed");

        emit SmgMintLoggerErc1155(params.uniqueID, params.smgID, params.tokenPairID, params.tokenID, params.value, params.fee, params.destTokenAccount, params.destUserAccount);
    }
    ...
    /// @param storageData              Cross storage data
    /// @param params                   parameters for storeman burn lock token on token shadow chain
    function smgReleaseErc1155(CrossTypesV1.Data storage storageData, RapiditySmgReleaseParams memory params)
    public
    {
        storageData.rapidityTxData.addRapidityTx(params.uniqueID);
        ...
        if (params.destTokenAccount == address(0)) {
            ...
            uint8 tokenCrossType = storageData.tokenManager.mapTokenPairType(params.tokenPairID);
            require(tokenCrossType == uint8(TokenCrossType.ERC1155), "Not ERC1155");
            IERC1155(params.destTokenAccount).safeTransferFrom(address(this), params.destUserAccount, params.tokenID, params.value, '0x0');
        }
        emit SmgReleaseLoggerErc1155(params.uniqueID, params.smgID, params.tokenPairID, params.tokenID, params.value, params.fee, params.destTokenAccount, params.destUserAccount);
    }
}
