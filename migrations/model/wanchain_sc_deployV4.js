...
async function deploy(cfg, isMainnet, options = {}) {
    ...
    await deployer.deploy(scDict.RapidityLibV4);
    let rapidityLib = await deployer.deployed(scDict.RapidityLibV4);
    contract[scDict.RapidityLib] = rapidityLib.address;

    await deployer.link(scDict.CrossDelegateV4, scDict.HTLCTxLib);
    await deployer.link(scDict.CrossDelegateV4, scDict.RapidityLibV4);
    ...
    await deployer.deploy(scDict.CrossDelegateV4);
    let crossDelegate = await deployer.deployed(scDict.CrossDelegateV4);
    ...
    let crossApproach = await deployer.at(scDict.CrossDelegateV4, crossProxy.address);
    try {
      if (crossAdmin) {
