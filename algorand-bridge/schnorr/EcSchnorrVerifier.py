...
def check_ecSchnorr_sig(
    ...
    *,
    ...
    rkec256 = pt.Keccak256(Concat(R, hv, px.get(), message.get()))
    ret = If(pt.BytesEq(e.get(), rkec256),  Int(1), Int(0))

    return pt.Seq(
        OpUp(mode=pt.OpUpMode.OnCall).ensure_budget(Int(3000),fee_source=pt.OpUpFeeSource.GroupCredit),
        output.set(ret),
    )