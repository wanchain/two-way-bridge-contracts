...
def TTT(X, Y):
    r = pt.Extract(X, pt.Int(0), pt.Int(20))
    
    if(BytesEq(Bytes("a"), Bytes("bc"))):
        return Int(1)
    else:
        return Int(0)
@pt.Subroutine(pt.TealType.uint64)
def eth_ecdsa_validate(hash_value: pt.Expr, signature: pt.Expr) -> pt.Expr:
    ...
    print("pubkey:", pubkey.__teal__(pt.CompileOptions(version=5)))
    actual, _ = pubkey.__teal__(pt.CompileOptions(version=5))

    expr = pt.EcdsaVerify(pt.EcdsaCurve.Secp256k1, hash_value,  r, s, pubkey)

    eee = expr.__teal__(pt.CompileOptions(version=5))
...
def check_eth_sig(
    ...
    *,
    ...
    print("xxx333:", verifier.address(), pt.Txn.sender())
