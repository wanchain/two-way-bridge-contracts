...
const web3url = "http://192.168.1.179:7654"
...
async function registerStart(smg, inheritGroupId = '', wlStartIndex = 0){
 ...
 //   let tx = await smg.storemanGroupRegisterStart(id, now+10, 7*24*3600, 60, preGroupId, wks,srs, {from: owner})
    let tx = await smg.storemanGroupRegisterStart(id, now+10, 7*24*3600, 6*24*3600, preGroupId, wks,srs, {from: owner})
