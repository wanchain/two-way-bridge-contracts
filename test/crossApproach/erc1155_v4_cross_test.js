...
exports.testCases = () => {
  async function getErc1155TokenInstance(tokenAccount) {
    let erc1155Inst = new web3.eth.Contract(
      WrappedErc1155Json.abi,
      tokenAccount
    );
    return erc1155Inst;
  }
  describe("Cross_ERC1155", function () {
    // NFT ...
    it("Chain [ETH] <=> Chain [WAN] -> TOKEN [ERC1155 @ethereum] <( ethereum => wanchain )> -> userLockNFT  ==> success", async () => {
      ...
      const currentToken = global.chains[currentChainType].erc1155Tokens.filter(
        (token) => token.symbol === "Erc1155Symbol"
      )[0];
      ...
      if (
        !new web3.utils.BN(currTokenCrossType).eq(
          new web3.utils.BN(erc1155TokenCrossType)
        )
      ) {
      ...
      let balances = await tokenInstance.methods
        .balanceOfBatch(senderAccounts, tokenIDs)
        .call();
      let idx;
      for (idx = 0; idx < tokenIDs.length; ++idx) {
        assert.equal(
          ...
          parseInt(balances[idx]),
          "before userLockNFT check balances:",
          balances
        );
      }

      // approve
      let setApprovalForAllReceipt = await tokenInstance.methods
        .setApprovalForAll(cross.address, true)
        .send({
          from: senderAccount,
      ...
      let approved = await tokenInstance.methods
        .isApprovedForAll(senderAccount, cross.address)
        .call();
      ...
      balances = await tokenInstance.methods
        .balanceOfBatch(senderAccounts, tokenIDs)
        .call();
      for (idx = 0; idx < balances.length; ++idx) {
        assert.equal(
          0,
          parseInt(balances[idx]),
          "after check userLockErc1155 balanceOf error idx:",
          idx,
          ",balance:",
          balances[idx]
        );
    ...
    it("Chain [ETH] <=> Chain [WAN] -> TOKEN [ERC1155 @wanchain] <( ethereum => wanchain )> -> smgMintNFT  ==>  success", async () => {
      ...
      const currentToken = global.chains[buddyChainType].erc1155Tokens.filter(
        (token) => token.symbol === "Erc1155Symbol"
      )[0];
      ...
      let balances = await tokenInstance.methods
        .balanceOfBatch(wanUserAccounts, tokenIDs)
        .call();
      for (idx = 0; idx < balances.length; ++idx) {
        assert.equal(
          Number(balances[idx]),
          Number(tokenValues[idx]),
          "after check smgMintNFT balanceOfBatch error:",
          balances
        );
      }
    });
    ...
    it("Chain [ETH] <=> Chain [WAN] -> TOKEN [ERC1155 @wanchain] <( wanchain => ethereum )> -> userBurnNFT ==>  success", async () => {
      ...
      let idx;
      // get token instance
      let tokenInstance = await getErc1155TokenInstance(
        funcParams.tokenAccount
      );
      let senderAccounts = [
        senderAccount,
      ...
      for (idx = 0; idx < tokenIDs.length; ++idx) {
        assert.equal(
          tokenValues[idx],
          parseInt(balances[idx]),
          "before userBurnNFT check balance:",
          balances
        );
      }

      // approve
      await tokenInstance.methods.setApprovalForAll(cross.address, true).send({
        from: senderAccount,
        gasPrice: "20000000000", // ganacle-cli default value
        gas: 6721975, // ganache-cli default value
      });
    ...
    it("Chain [ETH] <=> Chain [WAN] -> TOKEN [ERC1155 @ethereum] <( wanchain => ethereum )> -> smgReleaseNFT  ==>  success", async () => {
      ...
      let balances = await tokenInstance.methods
        .balanceOfBatch(userAccounts, tokenIDs)
        .call();
      for (idx = 0; idx < tokenIDs.length; ++idx) {
        assert.equal(
          0,
          parseInt(balances[idx]),
          "before smgReleaseNFT check balances:",
          balances
        );
      }
      ...
      let crossBalances = await tokenInstance.methods
        .balanceOfBatch(crossAddresses, tokenIDs)
        .call();
      for (idx = 0; idx < tokenIDs.length; ++idx) {
        assert.equal(
          new web3.utils.BN(crossBalances[idx]).eq(
            new web3.utils.BN(tokenValues[idx])
          ),
          true,
          "before userReleaseNFT check cross balance error"
        );
      }
      ...
      for (idx = 0; idx < tokenIDs.length; ++idx) {
        assert.equal(
          tokenValues[idx],
          parseInt(balances[idx]),
          "after userReleaseNFT check balances:",
          balances
        );
      }

      crossBalances = await tokenInstance.methods
        .balanceOfBatch(crossAddresses, tokenIDs)
        .call();
      for (idx = 0; idx < tokenIDs.length; ++idx) {
        assert.equal(
          new web3.utils.BN(crossBalances[idx]).eq(new web3.utils.BN(0)),
          true,
