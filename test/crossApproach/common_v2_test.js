...
// const { CrossDelegateV2 } = require("../../migrations/wan/contract");
const {
    ADDRESS_0,
...
it('Others getStoremanFee @wanchain and @ethereum  -> The config value', async () => {
    let wanCross = await CrossDelegateV2.at(global.chains[chainTypes.WAN].scAddr.CrossProxy);
    ...
    let ethCross = await CrossDelegateV2.at(global.chains[chainTypes.ETH].scAddr.CrossProxy);
...
it('Others getPartners @wanchain and @ethereum  ==> The chainID value', async () => {
    ...
    let wanCross = await CrossDelegateV2.at(global.chains[wanchain].scAddr.CrossProxy);
    ...
    let ethCross = await CrossDelegateV2.at(global.chains[ethereum].scAddr.CrossProxy);
...
it('Others getPartners @wanchain and @ethereum  ==> The config value', async () => {
    ...
    assert.equal(global.chains[wanchain].scAddr.QuotaProxy, wanPartners.quota, `check partners quota at ${wanchain} failed`);
    ...
    assert.equal(global.chains[ethereum].scAddr.QuotaProxy, ethPartners.quota, `check partners quota at ${ethereum} failed`);
...
it('Others getFees @wanchain and @ethereum  ==> The config value', async () => {
    ...
    fees = global.crossFeesV2[wanchain][ethereum];
    ...
    fees = global.crossFeesV2[ethereum][wanchain];
...
it('Proxy @wanchain   -> get the implementation address', async () => {
    ...
    assert.equal(address, global.chains[chainTypes.WAN].scAddr.CrossDelegateV2, "check implementation failed");
});
...
it('Proxy @wanchain   -> restore', async () => {
    let crossProxy = await CrossProxy.at(global.chains[chainTypes.WAN].scAddr.CrossProxy);
    let ret = await crossProxy.upgradeTo(global.chains[chainTypes.WAN].scAddr.CrossDelegateV2);
    let address = await crossProxy.implementation();
    assert.equal(address, global.chains[chainTypes.WAN].scAddr.CrossDelegateV2, "check implementation failed");
