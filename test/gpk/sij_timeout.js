...
contract('Gpk_UT_sij_timeout', async () => {
  ...
  // SijTimeout
  it('[GpkDelegate_SijTimeout] should fail: Not need', async () => {
    let result = {};
    try {
      await gpkSc.SijTimeout(groupId, 0, data.smList[0].address, {from: data.smList[0].address});
    } catch (e) {
      result = e;
    }
  ...
  it('[GpkDelegate_SijTimeout] should fail: Not late', async () => {
    let result = {};
    try {
  ...
  it('[GpkDelegate_SijTimeout] should success', async () => {
    let result = {};
    try {
      await sleep(15 * 1000);
      await gpkSc.SijTimeout(groupId, 0, data.smList[0].address, {from: data.smList[0].address});
