...
contract('Gpk_UT_gpk', async(accounts) => {
  ...
  it('[GpkDelegate_curve_3] should success', async () => {
    ...
    gpkShare = await gpkSc.getGpkShare(groupId, 0);
    assert.notEqual(gpkShare.gpkShare1, '');
    assert.notEqual(gpkShare.gpkShare2, '');
    gpk = await gpkSc.getGpk(groupId);
    assert.notEqual(gpk.gpk1, '');
    assert.notEqual(gpk.gpk2, '');
    
    info = await gpkSc.getGroupInfobyRoundCurve(groupId, 0, 2);
    assert.equal(info.curveStatus, GpkStatus.Complete);
    info = await gpkSc.getGroupInfo(groupId, 0);
