...
const { deploySmg,expectRevert,expectEvent, setupNetwork, g } = require('../base.js');
...
contract('TestSmg', async () => {
    ...
    it('T2 upgradeTo a valid address. should succeed', async ()=>{
      ...
      await expectEvent(smgProxy, tx, 'Upgraded', [g.leader])
    })
    it('T3 should failed. "Cannot upgrade to the same implementation"', async ()=>{
      ...
      tx = smgProxy.connect(g.tester).upgradeTo(g.leader)
