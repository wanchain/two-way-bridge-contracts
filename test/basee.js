...
let web3url, owner, leader, leaderPk,timeBase, sfs, wks;
...
async function registerStart(smg, wlStartIndex = 0, option={}){
    ...
    let registerDuration1 = option.registerDuration ? option.registerDuration:registerDuration;
    let gpkDuration1 =  option.gpkDuration ? option.gpkDuration:gpkDuration;
    let htlcDuration1 =  option.htlcDuration ? option.htlcDuration:htlcDuration;
    let preGroupId =  option.preGroupId ? option.preGroupId : utils.stringTobytes32("");

    let smgIn = {
        groupId: id,
        preGroupId: preGroupId,
        workTime:now+(registerDuration1+gpkDuration1)*timeBase,
        totalTime:htlcDuration1*timeBase,
        registerDuration: registerDuration1*timeBase,
