...
describe('Signature', () => {
    ...
    it('should verify success', async () => {
        ...
        const gpk_y: bigint = BigInt("0xaceaa17ffb7bfafe15e2c026801400564854c9839a1665b65f18b228dd55ebcd");
    ...
    it('should verify success from data generated by tool', async () => {
        const user1 = await blockchain.treasury('user0');

        let item  = {
            s: '0x34d87429e9b37c224467cc189a912471b3e448d81c10e67414b1caa07fa5165a',
            e: '0xb2aeaefa70ce463d6cde7bc03c4cab8e50cc311975b5a303520f1705a201b3ad',
            p: '0x1b',
            pkX: '0xcb54bc900646fe8de5c8db04e4120e38cb61b3e000ae37e4ecdaf71b777f7ec7',
            pkY: '0x1f81f87d21eb46e372105a3d123af9a94e0760f9c13738b8ca1abf248a9104f2',
            msgHash: '0x50173254f9780541f758b9a5c9e4266ced10fda660074c1f72e7d8565057e1ad'
        }

        const queryID=1;
        const verifyRet = await signature.sendVerify(user1.getSender(), {
            s:BigInt(item.s),
            gpk_x:BigInt(item.pkX),
            gpk_y:BigInt(item.pkY),
            e:BigInt(item.e),
            p:BigInt(item.p),
            msg:BigInt(item.msgHash),
            value: toNano('1000'),
            queryID,
        });
    ...
    it('should verify success from data generated by tool automatically', async () => {
        const user1 = await blockchain.treasury('user0');
        ...
        let item = schnorr.getSecSchnorrS(skSmg, typesArray, parameters);
        console.log("=====s===hex");
