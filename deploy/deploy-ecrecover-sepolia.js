...
async function main() {
    ...
    let EcSchnorrJacob = await hre.ethers.getContractFactory("EcSchnorrJacob");
    let ecSchnorrVerifier = await EcSchnorrJacob.deploy();
    if (waitForReceipt) {
    ...
    await verify(hre, EcSchnorrJacob, ecSchnorrVerifier.address)
}
